...
const sketch = () => {
  ...
  for (let i = 0; i < numCircles; i++) {
    rotationOffsets.push(random.range(Math.PI * -.25, Math.PI * .25));
...
const addListeners = () => {
  window.addEventListener("mouseup", () => {
    ...
    if (audio.paused) audio.play(), manager.play();
    else audio.pause(), manager.pause();
  })
};

const createAudio = () => {
  audio = document.createElement("audio");
  audio.src = "./src/audios/Yarin Primak - Wine Up.mp3";  
